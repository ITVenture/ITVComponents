...
namespace ITVComponents.WebCoreToolkit.EntityFramework.AspNetCoreTenants
{
    ...
    public class AspNetSecurityContext : IdentityDbContext, IForeignKeyProvider, ISecurityContext<string, User, Role,Permission,UserRole,RolePermission,TenantUser,NavigationMenu,TenantNavigationMenu,DiagnosticsQuery,DiagnosticsQueryParameter,TenantDiagnosticsQuery,DashboardWidget,DashboardParam,UserWidget, CustomUserProperty>
    {
        ...
        public AspNetSecurityContext(IPermissionScope tenantProvider, IContextUserProvider userProvider, ILogger<AspNetSecurityContext> logger, DbContextOptions<AspNetSecurityContext> options) : base(options)
        {
            this.logger = logger;
