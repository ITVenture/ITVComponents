...
{
    public abstract class BaseServer:IDeferredInit,IDisposable
    {
        ...
        /// </summary>
        protected BaseServer(IPluginFactory factory, bool useExtendedProxying, bool useSecurity, ICustomServerSecurity security)
            ...
            lock (resourceLock)
            {
                if (eventSubscriptions.ContainsKey(eventName))
                {
                    ...
                    for (int i = 0; i < arguments.Length; i++)
                    {
                        try
                        {
                            var uq = plugins.GetUniqueName(arguments[i]);
