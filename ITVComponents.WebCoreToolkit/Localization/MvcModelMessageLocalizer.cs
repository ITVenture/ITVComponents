...
{
    public class MvcModelMessageLocalizer : IConfigureOptions<MvcOptions>
{
    private readonly IServiceScopeFactory _serviceFactory;
    public MvcModelMessageLocalizer(IServiceScopeFactory serviceFactory)
    {
    ...
    public void Configure(MvcOptions options)
    {
        using(var scope = _serviceFactory.CreateScope())
        {
            ...
            var localizer = provider.GetRequiredService<IStringLocalizer<MvcModelMessageLocalizer>>();
           ...
           options.ModelBindingMessageProvider.SetValueMustNotBeNullAccessor((x) =>
               localizer["The value '{0}' must not be null.", x]);
        }
    }
}
}
