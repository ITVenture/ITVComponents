...
namespace ITVComponents.WebCoreToolkit.EntityFramework.Billing.Abstractions
{
    ...
    public sealed class SaleResult
    {
        ...
        /// <summary>
        /// E-mail for the receipt: what the host passed in at creation, or — where the provider's
        /// implementation is configured to capture it — what the end customer entered on the provider's payment
        /// page. Null until the sale is paid in the latter case, because that is when the address arrives.
        /// </summary>
        public string? CustomerEmail { get; set; }
