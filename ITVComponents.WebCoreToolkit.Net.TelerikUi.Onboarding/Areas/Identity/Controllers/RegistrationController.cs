...
using ITVComponents.WebCoreToolkit.Net.TelerikUi.Onboarding.Areas.Identity.DTO;
...
namespace ITVComponents.WebCoreToolkit.Net.TelerikUi.Onboarding.Areas.Identity.Controllers
{
                    ...
                    if (resolved != null)
                    {
                        ...
                        tenantInitializer.ApplyTemplate(tenant, mku, ct =>
                        {
                            ...
                            if (!string.IsNullOrEmpty(cfg.AdminUserRole))
                            {
                                ...
                                // FK scalar, not the navigation: 'admin' (and its Tenant nav) is tracked by 'dbContext',
                                // NOT by this template helper's separately-leased 'ctx'. Assigning it as a navigation makes
                                // EF treat both admin (TenantUsers) and its tenant (Tenants) as new principals and emit
                                // INSERTs with explicit identity values → "Cannot insert explicit value for identity column".
                                // 'admin' was already saved, so its PK is populated; only the FK scalar is needed.
