...
/// <summary>
/// Hierarchy-strategy implementation of <see cref="IOnboardingAdminHandler"/> over
/// <c>IHierarchySecurityContextWithOnboarding</c>. Behaves exactly like the flat variant (current-tenant
/// scope, per-area <c>Onboarding.Admin.*</c> + membership gate, filter-bypassing tenant-explicit queries)
/// but binds to the hierarchy onboarding entities.
/// </summary>
public class HierarchyOnboardingAdminHandler<TContext> : IOnboardingAdminHandler
    where TContext : DbContext, IHierarchySecurityContextWithOnboarding
{
    ...
    public HierarchyOnboardingAdminHandler(IDbContextFactory<TContext> dbFactory, UserManager<User> userManager, IServiceProvider services)
    {
    ...
    public async Task<int?> SaveBillingProfileAsync(ClaimsPrincipal admin, BillingProfileAdminViewModel model, CancellationToken ct = default)
    {
        ...
        HierarchyBillingProfile profile;
        ...
        else
        {
            ...
            profile = new HierarchyBillingProfile { TenantId = current, OwnerUserId = owner?.Id };
    ...
    public async Task<int?> SaveEmployeeAsync(ClaimsPrincipal admin, EmployeeViewModel model, CancellationToken ct = default)
    {
        ...
        {
            employee = new HierarchyEmployee { BillingProfileId = model.BillingProfileId, TenantId = current };
    ...
    public async Task<bool> SetEmployeeRoleAsync(ClaimsPrincipal admin, int employeeId, int employeeRoleMappingId, bool assigned, CancellationToken ct = default)
    {
        ...
        {
            db.EmployeeRoles.Add(new HierarchyEmployeeRole { EmployeeId = employeeId, EmployeeRoleMappingId = employeeRoleMappingId });
            await db.SaveChangesAsync(ct);
        }
    ...
    public async Task<int?> SaveRoleMappingAsync(ClaimsPrincipal admin, EmployeeRoleMappingViewModel model, CancellationToken ct = default)
    {
        ...
        HierarchyEmployeeRoleMapping mapping;
        ...
        else
        {
            ...
            mapping = new HierarchyEmployeeRoleMapping { TenantId = current, RoleId = roleId };
    ...
    /// possibly inherited — access on the tenant, so no separate membership probe is needed. (Probing direct
    /// membership here would even be wrong: it would reject a caller whose permission is inherited from a
    /// parent tenant.) When no permissions are supplied, falls back to "any onboarding-admin permission".
    /// </summary>
    ...
    private static AddressInput ToInput(HierarchyAddress? address) => address == null
        ? new AddressInput()
        : new AddressInput
        {
            Name = address.Name,
