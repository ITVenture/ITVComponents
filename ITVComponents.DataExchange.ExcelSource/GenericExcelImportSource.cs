...
{
    public abstract class GenericExcelImportSource:DictionarySourceBase<ExcelDataRecord>
    {
        /// <summary>
        ...
        private bool readNullData;

        /// <summary>
        /// Initializes a new instance of the GenericExcelImportSource class
        /// </summary>
        /// <param name="fileName"></param>
        protected GenericExcelImportSource(string fileName, bool readNullData)
        {
            sourceFile = fileName;
            this.readNullData = readNullData;
        }
                ...
                if ((ImportWorkSheetDecider.Decide(sheet.SheetName, DecisionMethod.Simple, out msg) & (DecisionResult.Acceptable | DecisionResult.Success)) != DecisionResult.None)
                {
                    ...
                    yield return
                        new Dictionary<string, ExcelDataRecord>
                        {
                            {
                                "SheetName",
                                new ExcelDataRecord {FormattedText = sheet.SheetName, RawData = sheet.SheetName}
                            }
                        };
                            ...
                            foreach (ICell cell in row.Cells)
                            {
                                ...
                                if (value != null)
                                {
                                    vals.Add(num.ToString(),
                                        new ExcelDataRecord {RawData = value, FormattedText = f.FormatCellValue(cell)});
                                }
                                else if (readNullData)
                                {
                                    vals.Add(num.ToString(), new ExcelDataRecord {RawData = null, FormattedText = ""});
                                }
                            }

                            string origin = $"{sheet.SheetName}::{row.RowNum}";
                            vals.Add("$origin", new ExcelDataRecord{ FormattedText = origin, RawData = origin});
                            yield return vals;
                        }
                    }
                }
