...
(a.tenantId=@tenantId or (a.tenantId is null and @tenantId is null))"
                            ...
                            db.GetParameter("tenantId",tenantName));
                    var joined = data.Select(d => new GenericTypeDefinition(){ TypeParameterName = (string)d["GenericTypeName"], TypeExpression = (string)d["TypeExpression"] }).ToArray();
                    genericTypeArguments.BuildGenericArguments(uniqueName, joined, customVariables, formatter);
                }
            }
        }
