...
{
    [Authorize("HasPermission(Tenants.View,Tenants.Write,Tenants.AssignUser,Tenants.WriteSettings,Tenants.AssignNav,Tenants.AssignQuery),HasFeature(ITVAdminViews)"), Area("Security"), ConstructedGenericControllerConvention(ControllerName = "TenantController"), CustomGenericTypeArg("TTenantViewModel",typeof(TenantViewModel)), CustomGenericTypeArg("TTenantSettingViewModel", typeof(TenantSettingViewModel))]
    public class TenantControllerClass<TTenant, TUserId, TUser, TRole, TPermission, TUserRole, TRolePermission, TTenantUser, TRoleRole, TGlobalRole, TGlobalRolePermission, TGRoleLRole, TNavigationMenu, TTenantNavigation, TQuery, TQueryParameter, TTenantQuery, TWidget, TWidgetParam, TWidgetLocalization, TUserWidget, TUserProperty, TAssetTemplate, TAssetTemplatePath, TAssetTemplateGrant, TAssetTemplateFeature, TSharedAsset, TSharedAssetUserFilter, TSharedAssetTenantFilter, TClientAppTemplate, TAppPermission, TAppPermissionSet, TClientAppTemplatePermission, TClientApp, TClientAppPermission, TClientAppAccess, TWebPlugin, TWebPluginConstant, TWebPluginGenericParameter, TSequence, TTenantSetting, TTenantFeatureActivation, TExternalOAuthService, TExternalOAuthServiceState, TExternalOAuthServiceTenantLogin, TTrustConfig, TContext, TTenantViewModel, TTenantSettingViewModel> : Controller
        ...
        where TTenantSettingViewModel: TenantSettingViewModel, new()
        ...
        where TExternalOAuthServiceTenantLogin : ExternalOAuthServiceTenantLogin<TTenant, TExternalOAuthService, TExternalOAuthServiceState, TExternalOAuthServiceTenantLogin>
    {
        ...
        public TenantControllerClass(TContext db, IUserExpressionHelper<TUserId, TUser, TTenantUser, TRoleRole> expressionHelper, IServiceProvider services, IOptions<SecurityViewsOptions> options)
        {
        ...
        public IActionResult Read([DataSourceRequest] DataSourceRequest request,[FromQuery] TUserId userId, [FromQuery]int? navigationMenuId, [FromQuery]int? diagnosticsQueryId)
        {
            ...
            if (userId != null)
            {
                return Json((from p in db.Tenants
                    join r in db.TenantUsers on new { p.TenantId, UserId = userId } equals new { r.TenantId, r.UserId }
                        into lj
            ...
            TTenantViewModel viewModel)
        {
            ...
            return Json(
                await new[] { model.ToViewModel<TTenant, TTenantViewModel>() }.ToDataSourceResultAsync(request,
                    ModelState));
        }
        ...
        {
            return Json(db.TenantSettings.Where(n => n.TenantId == tenantId).ToDataSourceResult(request, n => n.ToViewModel<TTenantSetting, TTenantSettingViewModel>()));
        }
        ...
        public async Task<IActionResult> CreateSetting([DataSourceRequest] DataSourceRequest request,[FromQuery] int tenantId)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                await this.TryUpdateModelAsync<TTenantSettingViewModel, TTenantSetting>(model);
            ...
            return Json(await new[] {model.ToViewModel<TTenantSetting, TTenantSettingViewModel>()}.ToDataSourceResultAsync(request, ModelState));
        }

        [HttpPost]
        [Authorize("HasPermission(Tenants.WriteSettings)")]
        public async Task<IActionResult> UpdateSetting([DataSourceRequest] DataSourceRequest request, TTenantSettingViewModel viewModel)
        {
            var model = db.TenantSettings.First(n => n.TenantSettingId == viewModel.TenantSettingId);
            if (ModelState.IsValid)
            {
                await this.TryUpdateModelAsync<TTenantSettingViewModel, TTenantSetting>(model, "", m => { return m.ElementType == null; });
                if (model.JsonSetting)
                {
        ...
        [HttpPost]
        [Authorize("HasPermission(Tenants.WriteSettings)")]
        public async Task<IActionResult> DestroySetting([DataSourceRequest] DataSourceRequest request, TTenantSettingViewModel viewModel)
        {
            var model = db.TenantSettings.First(n => n.TenantSettingId == viewModel.TenantSettingId);
            if (ModelState.IsValid)
            {
