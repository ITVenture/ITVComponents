    ...
    where TContext : DbContext, IHierarchySecurityContextWithOnboarding
{
    ...
    {
        var membership = await GetMembershipAsync(admin, input.ParentTenantId, ct);
        if (membership == null)
        {
    ...
    /// </summary>
    private async Task<HierarchyTenantUser> GetMembershipAsync(ClaimsPrincipal admin, int tenantId, CancellationToken ct)
    {
        var owner = await userManager.GetUserAsync(admin);
        if (owner == null)
        {
