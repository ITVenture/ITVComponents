...
{
    public abstract class DbSecurityRepository<TTenant, TUserId, TUser, TRole, TPermission, TUserRole, TRolePermission, TTenantUser, TRoleRole, TGlobalRole, TGlobalRolePermission, TGRoleLRole, TNavigationMenu, TTenantNavigation, TQuery, TQueryParameter, TTenantQuery, TWidget, TWidgetParam, TWidgetLocalization, TUserWidget, TUserProperty, TAssetTemplate, TAssetTemplatePath, TAssetTemplateGrant, TAssetTemplateFeature, TSharedAsset, TSharedAssetUserFilter, TSharedAssetTenantFilter, TClientAppTemplate, TAppPermission, TAppPermissionSet, TClientApp, TClientAppPermission, TClientAppAccess, TWebPlugin, TWebPluginConstant, TWebPluginGenericParameter, TSequence, TTenantSetting, TTenantFeatureActivation, TExternalOAuthService, TExternalOAuthServiceState, TExternalOAuthServiceTenantLogin, TTrustConfig> : ISecurityRepository
        ...
        where TExternalOAuthServiceTenantLogin : ExternalOAuthServiceTenantLogin<TTenant, TExternalOAuthService, TExternalOAuthServiceState, TExternalOAuthServiceTenantLogin>, new()
    {
        ...
        /// </summary>
        private IContextLease<ISecurityContext<TTenant, TUserId, TUser, TRole, TPermission, TUserRole, TRolePermission, TTenantUser, TRoleRole, TGlobalRole, TGlobalRolePermission, TGRoleLRole, TNavigationMenu, TTenantNavigation, TQuery, TQueryParameter, TTenantQuery, TWidget, TWidgetParam, TWidgetLocalization, TUserWidget, TUserProperty, TAssetTemplate, TAssetTemplatePath, TAssetTemplateGrant, TAssetTemplateFeature, TSharedAsset, TSharedAssetUserFilter, TSharedAssetTenantFilter, TClientAppTemplate, TAppPermission, TAppPermissionSet, TClientApp, TClientAppPermission, TClientAppAccess, TWebPlugin, TWebPluginConstant, TWebPluginGenericParameter, TSequence, TTenantSetting, TTenantFeatureActivation, TExternalOAuthService, TExternalOAuthServiceState, TExternalOAuthServiceTenantLogin, TTrustConfig>> LeaseContext()
        ...
        public void EnsureRequestedPermissions(string[] permissionNames, AutoPermissionsOptions options)
        {
            ...
            try
            {
                ...
                if (dirty)
                {
                    ...
                    AutoPermissionWriteGuard.SaveIdempotent(securityContext as DbContext, logger,
        ...
        /// </summary>
        /// <remarks>
        /// An EINER Stelle formuliert, weil sie in beiden <c>GetPermissions</c>-Fassungen und dort je
        /// zweimal gebraucht wird - einmal als Deckel fuer die Delegation, einmal als das ganze Recht
        /// einer Maschine.
        /// </remarks>
        /// <summary>
        ...
        public virtual string[] GetGlobalRoles(string[] userLabels, string userAuthenticationType)
        {
            ...
            {
                // ACHTUNG, gilt fuer alle ACHT Vorkommen dieses Musters in dieser Datei: die Bezeichner werden
                // HIER kleingeschrieben, und die Abfrage unten vergleicht gegen Label.ToLower(). Die bequeme
                // Fassung Contains(wert, StringComparer.OrdinalIgnoreCase) kann EF Core NICHT uebersetzen - es
                // versucht dann, den Ausdruck als Parameter auszuwerten, und der Interpreter scheitert auf
                // .NET 10 an der params-ReadOnlySpan-Gestalt. Die Meldung nennt weder Contains noch die Tabelle,
                // und der InMemory-Provider wertet client-seitig aus - kein Test dagegen findet das je.
                ...
                // Der Mandant haengt an der APP, nicht am Benutzer: seit TenantUserId optional ist,
                // faehrt der Umweg ueber den Benutzer bei einem Maschinenzugang ins Leere.
                ...
                tenantUsers = appUsers.Where(n => n.TenantUserId != null).Select(n => n.TenantUser.User);
            }
        ...
        private bool IsAuthenticatedCore(string[] userLabels, string userAuthenticationType, int? t, ISecurityContext<TTenant, TUserId, TUser, TRole, TPermission, TUserRole, TRolePermission, TTenantUser, TRoleRole, TGlobalRole, TGlobalRolePermission, TGRoleLRole, TNavigationMenu, TTenantNavigation, TQuery, TQueryParameter, TTenantQuery, TWidget, TWidgetParam, TWidgetLocalization, TUserWidget, TUserProperty, TAssetTemplate, TAssetTemplatePath, TAssetTemplateGrant, TAssetTemplateFeature, TSharedAsset, TSharedAssetUserFilter, TSharedAssetTenantFilter, TClientAppTemplate, TAppPermission, TAppPermissionSet, TClientApp, TClientAppPermission, TClientAppAccess, TWebPlugin, TWebPluginConstant, TWebPluginGenericParameter, TSequence, TTenantSetting, TTenantFeatureActivation, TExternalOAuthService, TExternalOAuthServiceState, TExternalOAuthServiceTenantLogin, TTrustConfig> securityContext)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    // pruefen koennte, und der Zugang selbst ist bereits auf Gueltigkeit geprueft.
        ...
        public virtual IEnumerable<CustomUserProperty> GetCustomProperties(string[] userLabels, string userAuthenticationType, CustomUserPropertyType propertyType)
        {
            ...
            else
            {
                var filteredLabels = (from ul in userLabels
                    where Regex.IsMatch(ul, Global.AppUserKeyPattern)
                    select Regex.Match(ul, Global.AppUserKeyPattern).Groups["appUserKey"].Value.ToLowerInvariant()).ToArray();
                // Ein Maschinenzugang hat keinen Benutzer und damit auch keine Benutzereigenschaften.
        ...
        public virtual IEnumerable<Permission> GetPermissions(string[] userLabels, string userAuthenticationType)
        {
            ...
            string[] machinePerms = null;
            // Festgehalten statt inline geprueft: der leere Treffer unten muss sagen koennen, WELCHER der
            // beiden Wege gelaufen ist - das ist die Information, die die Fehlersuche abkuerzt.
            ...
            else
            {
                var filteredLabels = (from ul in userLabels
                    where Regex.IsMatch(ul, Global.AppUserKeyPattern)
                    select Regex.Match(ul, Global.AppUserKeyPattern).Groups["appUserKey"].Value.ToLowerInvariant()).ToArray();
                // BUGFIX: der Deckel wurde frueher ueber ALLE Zugaenge des Mandanten gebildet, nicht
                // ueber die gesuchten - App A hob damit den Deckel fuer App B. Der Label-Filter gehoert
                // VOR die Buendel-Aufloesung.
                var appUsers = securityContext.ClientAppAccesses
                    .Where(n => n.ClientApp.TenantId == securityContext.CurrentTenantId.Value && n.RevokedUtc == null && (n.ExpiresUtc == null || n.ExpiresUtc > DateTime.UtcNow) && n.ClientApp.Enabled)
                    .Where(au => filteredLabels.Contains(au.Label.ToLower()));
            ...
            {
                // Maschine: die Buendel der App gelten DIREKT - kein Benutzer, kein Schnitt. Ein
                // Kassenterminal hat keine Rollen, aus denen sich etwas schneiden liesse.
            ...
            {
                LogEmptyPermissionResolution(userLabels, isUser);
            }
        ...
        public virtual IEnumerable<ScopeInfo> GetEligibleScopes(string[] userLabels, string authType)
        {
            ...
            if (userLabels.Any(n => Regex.IsMatch(n, Global.AppUserKeyPattern)))
            {
                var filteredLabels = (from ul in userLabels
                    where Regex.IsMatch(ul, Global.AppUserKeyPattern)
                    select Regex.Match(ul, Global.AppUserKeyPattern).Groups["appUserKey"].Value.ToLowerInvariant()).ToArray();
                // BUGFIX: filteredLabels wurde frueher berechnet und nie benutzt - JEDES App-Label
                // lieferte damit die Mandanten ALLER Zugaenge. Und der Mandant kommt jetzt von der App,
                // weil ein Maschinenzugang keinen Benutzer hat.
                return (from d in securityContext.ClientAppAccesses
                        .Where(n => filteredLabels.Contains(n.Label.ToLower()))
